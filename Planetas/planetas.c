...
void display(void){
	...
	// Desenhar planeta rosa e suas luas
	glPushMatrix();
	    glRotatef ((GLfloat) -year, 0.0, 1.0, 0.0);
	    glTranslatef (2.0, 1.0, -1.0);
	    glRotatef ((GLfloat) -day, 0.0, 1.0, 0.0);

	    // Desenha o planeta rosa
	    glColor3f (1.0, 0, 1.0);
	    glutWireSphere(0.2, 10, 8);
	    ...
	    glPushMatrix();
	        glColor3f (1.0, 1.0, 1.0);
	        glRotatef((GLfloat) year * 5, 1.0, 1.0, 0.0);
	        glTranslatef(0.4, 0.0, 0.0);
	        glRotatef ((GLfloat) day * 5, 0.0, 1.0, 0.0);
	        glutWireSphere(0.05, 10, 8); // Desenha a lua
	    glPopMatrix();

	    // Lua rosa claro
	    glPushMatrix();
	        glColor3f (1.0, 0.5, 1.0);
	        glRotatef((GLfloat) year * 5, 1.0, 0.0, 0.0);
